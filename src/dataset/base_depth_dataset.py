...
class BaseDepthDataset(Dataset):
    def __init__(
        ...
        **kwargs,
        ...
        self.dataset_dir = base_data_dir
    ...
    def __getitem__(self, index):
        ...
        outputs['cls'] = torch.tensor([self.class_embed])
        if self.sota_flag:
            return {
                'rgb': outputs['rgb_norm'],
                'prompt': outputs['valid_mask_raw'].to(torch.float32).repeat(3,1,1),
                'gt': outputs['depth_raw_norm'].repeat(3,1,1),
                'label': torch.tensor([1000]),
            }
        else:
            return outputs
    ...
    def _get_data_path(self, index):
        ...
        if self.base_data_dir:
            rgb_rel_path = os.path.join(self.base_data_dir, rgb_rel_path)

        depth_rel_path, filled_rel_path = None, None
        if DatasetMode.RGB_ONLY != self.mode:
            depth_rel_path = filename_line[1]
            if self.base_data_dir:
                depth_rel_path = os.path.join(self.base_data_dir, depth_rel_path)

            if self.has_filled_depth:
                filled_rel_path = filename_line[2]
                if self.base_data_dir:
                    filled_rel_path = os.path.join(self.base_data_dir, filled_rel_path)
